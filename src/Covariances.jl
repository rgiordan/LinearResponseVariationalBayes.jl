...
""" ->
function get_wishart_sufficient_stats_variational_covariance(lambda_suff, log_det_lambda_suff,
	                                                         lambda_i, log_det_lambda_i, ud_ind)
	@assert size(lambda_suff, 1) == size(lambda_suff, 2) == size(ud_ind, 1) == size(ud_ind, 2)
	...
	lambda_cov = get_wishart_variational_covariance(k_tot, v0, wn, ud_ind)
