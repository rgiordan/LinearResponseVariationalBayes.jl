...
function test_asymmetric_hessian_reparameterization()
	# An example function and transform.
	# I'm checking for a possible bug in whether the inverse Jacobian or
	# its transpose should be used.
	# An example function and transform.
	K = 2
	a = 1.5
	...
	df_dy = df_dy_func(y)
	d2f_dy2 = d2f_dy2_func(y)
	dx_dy = dx_dy_func(y) # Actually the transpose of the Jacobian
	d2x_dy2 = get_d2x_dy2(d2x_dy2_funcs, y)
...
# This will be failing until ForwardDiff.jl releases v0.0.3 to METADATA.
